...
async def get_theme_css(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(AppSetting).where(AppSetting.key.in_(["PRIMARY_COLOR", "SECONDARY_COLOR"])))
    settings_list = result.scalars().all()
    colors = {s.key: s.value for s in settings_list}
    
    primary = colors.get("PRIMARY_COLOR", "#8b5cf6")
    secondary = colors.get("SECONDARY_COLOR", "#3b82f6")
...
async def book_appointment(data: BookAppointmentReq, db: AsyncSession = Depends(get_db)):
    ...
    # TODO: Implement send_appointment_emails_async in email_engine.py
    print(f"Appointment booked for {data.name} on {data.date} at {data.time_slot}")
...
async def test_wa(data: WATestReq, db: AsyncSession = Depends(get_db)):
    ...
    try:
        success, err = await send_whatsapp_message(
            ...
            waba_id=client.whatsapp_business_account_id,
