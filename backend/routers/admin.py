...
async def get_credentials(admin = Depends(require_super_admin)):
    return {
        "GOOGLE_CLIENT_ID": settings.GOOGLE_CLIENT_ID,
        "GOOGLE_CLIENT_SECRET": settings.GOOGLE_CLIENT_SECRET,
        "GOOGLE_SERVICE_ACCOUNT_JSON": getattr(settings, "GOOGLE_SERVICE_ACCOUNT_JSON", ""),
...
async def update_credentials(creds: CredentialsUpdate, admin = Depends(require_super_admin)):
    import os
    import re
    ...
    update_dict = creds.dict(exclude_unset=True)
    for key, val in update_dict.items():
        if val is None: continue
        if key == "GOOGLE_SERVICE_ACCOUNT_JSON":
            pattern = re.compile(rf'({key}:\s*str\s*=\s*\"\"\")(.*?)(\"\"\")', re.DOTALL)
            content = pattern.sub(rf'\g<1>{val}\g<3>', content)
        else:
            pattern = re.compile(rf'({key}:\s*str\s*=\s*\")[^\"]*(\")')
            content = pattern.sub(rf'\g<1>{val}\g<2>', content)

    with open(config_path, "w", encoding="utf-8") as f:
        f.write(content)

    return {"status": "success", "message": "Credentials updated, server restarting"}
...
@router.get("/credentials")
async def get_system_credentials(admin = Depends(require_super_admin)):
    return {
        "JWT_SECRET": settings.JWT_SECRET,
        "GOOGLE_CLIENT_ID": settings.GOOGLE_CLIENT_ID,
        "GOOGLE_CLIENT_SECRET": settings.GOOGLE_CLIENT_SECRET,
...
@router.put("/credentials")
async def update_system_credentials(payload: CredentialsPayload, db: AsyncSession = Depends(get_db), admin = Depends(require_super_admin)):
    try:
        from backend.models.app_settings import AppSetting
        ...
        updates = {
            ...
            'GCP_CREDENTIALS_JSON': payload.GOOGLE_SERVICE_ACCOUNT_JSON
        }
        ...
        for key, value in updates.items():
            if value is not None and value.strip() != "":
                ...
                if setting:
                    ...
                    new_setting = AppSetting(key=key, value=value, category='system')
                    db.add(new_setting)
                
                # Update in-memory settings
                if key == 'GCP_CREDENTIALS_JSON':
                    settings.GOOGLE_SERVICE_ACCOUNT_JSON = value
                else:
                    setattr(settings, key, value)
                    
        await db.commit()
        return {"message": "Credentials updated successfully in DB and memory."}
    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=500, detail=str(e))
