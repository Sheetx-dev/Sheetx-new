...
async def get_dashboard(db: AsyncSession = Depends(get_db), current_user = Depends(require_client)):
    ...
    try:
        ...
        print(f"Error fetching campaign counts: {e}")
...
@router.post("/logo")
async def upload_client_logo(file: UploadFile = File(...), db: AsyncSession = Depends(get_db), current_user = Depends(require_active_subscription)):
    client = await get_client_profile(current_user, db)
    
    contents = await file.read()
    b64 = base64.b64encode(contents).decode("utf-8")
    mime = file.content_type or "image/png"
    client.logo_url = f"data:{mime};base64,{b64}"
    
    await db.commit()
    return {"status": "success", "logo_url": client.logo_url}
