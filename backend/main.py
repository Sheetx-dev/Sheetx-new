...
async def lifespan(app: FastAPI):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 2. Run migrations (Skipped to speed up startup - DB already migrated)
            ...
            #         print(f"Migration error: {e}")
            #         await conn.rollback()
            print("Schema migration skipped (already migrated)!")
            break # Success, break out of retry loop
        except Exception as e:
            error_msg = str(e).encode('ascii', 'ignore').decode()
            print(f"Schema migration error on attempt {attempt + 1}: {error_msg}")
            if attempt < max_retries - 1:
                await asyncio.sleep(3)

    # Seed Policies (Removed as requested to not store data in code)
    # Seed Credentials & Inject into settings
    try:
        async with engine.begin() as conn:
            ...
            keys_to_sync = [
                ...
                'RAZORPAY_KEY_ID', 'RAZORPAY_KEY_SECRET', 'GROQ_API_KEY',
                'GCP_CREDENTIALS_JSON'
            ]
            for key in keys_to_sync:
                res = await conn.execute(text("SELECT value FROM app_settings WHERE key = :key"), {"key": key})
                row = res.fetchone()
                if row and row[0]:
                    if key == 'GCP_CREDENTIALS_JSON':
                        setattr(app_settings, 'GOOGLE_SERVICE_ACCOUNT_JSON', row[0])
                    else:
                        setattr(app_settings, key, row[0])
                else:
                    # Seed with default from config if missing
                    default_val = getattr(app_settings, key if key != 'GCP_CREDENTIALS_JSON' else 'GOOGLE_SERVICE_ACCOUNT_JSON', "")
                    if default_val:
                        import uuid
                        await conn.execute(
                            text("INSERT INTO app_settings (id, key, value, category) VALUES (:id, :key, :val, 'system') ON CONFLICT DO NOTHING"),
                            {"id": str(uuid.uuid4()), "key": key, "val": default_val}
                        )
    except Exception as e:
        print(f"Failed to sync credentials from DB: {e}")
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[settings.APP_URL, settings.FRONTEND_URL, "http://localhost:3000", "http://localhost:8000"],
...
if __name__ == "__main__":
    import uvicorn
    # When run directly, start the server on all interfaces at port 8000
