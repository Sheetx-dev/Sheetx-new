...
async def process_single_campaign(campaign, groq_key):
    ...
    try:
        async with SessionLocal() as db:
            ...
            remaining_hourly_capacity = campaign.max_emails_per_hour - emails_sent_last_hour
            if remaining_hourly_capacity <= 0:
            ...
            # Enforce limits
            tasks_to_process = tasks_to_process[:remaining_hourly_capacity]
            if not tasks_to_process:
                pass # Nothing to do, but we should still update last_run_at below

            sheet_updates = []
            semaphore = asyncio.Semaphore(5) # Process up to 5 concurrent emails
            
            async def _process_lead(task_data):
                async with semaphore:
                    # Random Jitter to avoid spam filters
                    await asyncio.sleep(random.uniform(1.5, 4.0))
                    
                    email = task_data["email"]
                    name = task_data["name"]
                    phone = task_data["phone"]
                    target_template = task_data["target_template"]
                    category = task_data["category"]
                    is_follow_up_run = task_data["is_follow_up"]
                    ...
                    if getattr(campaign, 'review_mode', False):
                        ...
                        async with SessionLocal() as db:
                            ...
                            db.add(queue_item)
                            await db.commit()
                        sheet_updates.append({'row': row_index, 'col': status_idx + 1, 'value': 'Queued'})
                        return
                    ...
                    sheet_updates.append({'row': row_index, 'col': status_idx + 1, 'value': new_status})

            if tasks_to_process:
                await asyncio.gather(*[_process_lead(task) for task in tasks_to_process])
                
                if sheet_updates:
                    try:
                        await update_sheet_cells_batch(campaign.google_sheet_id, sheet_updates)
                    except Exception as e:
                ...
                for q in queued_emails:
                    ...
                    if success:
                        ...
                        try:
                            ...
                            print(f"Google Sheet update failed for {q.recipient_email}: {e}")
                    else:
                        q.status = "failed"
                        q.error_message = err
