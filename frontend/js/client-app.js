document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (profileData && profileData.logo_url) {
            const logoEl = document.getElementById('client-portal-logo');
            if (logoEl) logoEl.src = profileData.logo_url;
        }
    ...
    async function loadDashboard(isBackground = false) {
        try {
            ...
            if (el('dash-lifetime')) el('dash-lifetime').textContent = data.total_emails_sent || 0;
            if (el('dash-templates')) el('dash-templates').textContent = templatesReq ? templatesReq.length : 0;
            if (el('dash-total')) el('dash-total').textContent = data.total_emails_sent || 0;
            if (el('dash-campaigns')) el('dash-campaigns').textContent = data.active_campaigns || 0;
    ...
    document.getElementById('client-logo-upload')?.addEventListener('change', async (e) => {
        var file = e.target.files[0];
        if (!file) return;

        // Find the closest label or button to show loading state if possible
        const label = e.target.closest('label');
        const oldHtml = label ? label.innerHTML : '';
        if (label) {
            label.innerHTML = '<span class="loading loading-spinner loading-sm text-primary"></span> Uploading...';
        ...
        try {
            var token = api.getToken();
            var res = await fetch('/api/client/logo', {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + token },
                body: formData
            });
            ...
            if (window.showToast) showToast('Logo uploaded!', 'success');
        } catch (err) {
            if (window.showToast) showToast(err.message, 'error');
        } finally {
            if (label) {
