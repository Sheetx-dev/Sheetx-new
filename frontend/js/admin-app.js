document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.getCurrentUser().then(user => {
        ...
        router.on('credentials', loadCredentials);
    ...
    const primaryColor = document.getElementById('admin-landing-color-primary');
    const primaryText = document.getElementById('admin-landing-color-primary-text');
    const secondaryColor = document.getElementById('admin-landing-color-secondary');
    const secondaryText = document.getElementById('admin-landing-color-secondary-text');

    if (primaryColor && primaryText) {
        primaryColor.addEventListener('input', (e) => primaryText.value = e.target.value);
        primaryText.addEventListener('input', (e) => primaryColor.value = e.target.value);
    }
    if (secondaryColor && secondaryText) {
        secondaryColor.addEventListener('input', (e) => secondaryText.value = e.target.value);
        secondaryText.addEventListener('input', (e) => secondaryColor.value = e.target.value);
    }
    ...
    async function loadSettings() {
        try {
            ...
            settings.forEach(s => {
                ...
                const primaryEl = document.getElementById('admin-landing-color-primary');
                const primaryTextEl = document.getElementById('admin-landing-color-primary-text');
                const secondaryEl = document.getElementById('admin-landing-color-secondary');
                const secondaryTextEl = document.getElementById('admin-landing-color-secondary-text');
                ...
                if(s.key === 'PRIMARY_COLOR' && primaryEl) { primaryEl.value = s.value; primaryTextEl.value = s.value; }
                if(s.key === 'SECONDARY_COLOR' && secondaryEl) { secondaryEl.value = s.value; secondaryTextEl.value = s.value; }
            });
        } catch(e) {}
    ...
    document.getElementById('form-admin-settings')?.addEventListener('submit', async(e) => {
        ...
        if(primaryEl) payload.push({key: 'PRIMARY_COLOR', value: primaryEl.value, category: 'landing'});
        if(secondaryEl) payload.push({key: 'SECONDARY_COLOR', value: secondaryEl.value, category: 'landing'});
        try {
    ...
    async function loadCredentials() {
        try {
            const creds = await api.get('/admin/credentials');
            const el = (id) => document.getElementById(id);
            if(el('cred-jwt')) el('cred-jwt').value = creds.JWT_SECRET || '';
            if(el('cred-google-id')) el('cred-google-id').value = creds.GOOGLE_CLIENT_ID || '';
            if(el('cred-google-secret')) el('cred-google-secret').value = creds.GOOGLE_CLIENT_SECRET || '';
            if(el('cred-rzp-id')) el('cred-rzp-id').value = creds.RAZORPAY_KEY_ID || '';
            if(el('cred-rzp-secret')) el('cred-rzp-secret').value = creds.RAZORPAY_KEY_SECRET || '';
            if(el('cred-groq')) el('cred-groq').value = creds.GROQ_API_KEY || '';
            if(el('cred-gcp-json')) el('cred-gcp-json').value = creds.GOOGLE_SERVICE_ACCOUNT_JSON || '';
        } catch(e) {
            console.error("Load credentials error:", e);
        }
    }

    document.getElementById('form-admin-credentials')?.addEventListener('submit', async(e) => {
        e.preventDefault();
        if (!confirm("Are you sure you want to update the system credentials? Incorrect values will break core features.")) return;
        ...
        const payload = {
            JWT_SECRET: document.getElementById('cred-jwt')?.value,
            GOOGLE_CLIENT_ID: document.getElementById('cred-google-id')?.value,
            GOOGLE_CLIENT_SECRET: document.getElementById('cred-google-secret')?.value,
            RAZORPAY_KEY_ID: document.getElementById('cred-rzp-id')?.value,
            RAZORPAY_KEY_SECRET: document.getElementById('cred-rzp-secret')?.value,
            GROQ_API_KEY: document.getElementById('cred-groq')?.value,
            GOOGLE_SERVICE_ACCOUNT_JSON: document.getElementById('cred-gcp-json')?.value
        };

        try {
            await api.put('/admin/credentials', payload);
            if(window.showToast) showToast("Credentials saved successfully!", "success");
        } catch(err) { 
            if(window.showToast) showToast(err.message, "error"); 
        } finally {
            if (btn) {
